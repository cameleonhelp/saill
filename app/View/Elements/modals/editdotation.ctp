<?php $modaltitle = "Modifier la dotation de matériel"; ?>
<!--modal hebdomadaire//-->
<div class="modal fade" id="modaleditdotation" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="block-content">
            ...
            <?php echo $this->Form->create('Dotation',array('action'=>'editto','id'=>'formValidate','class'=>'form-horizontal','inputDefaults' => array('error'=>false,'label'=>false,'div' => false))); ?>
    ...
    <div class="modal-footer">
      <button type="button" class="btn btn-sm btn-default" id="closemodaleditdotation">Annuler</button><button type="submit" class="btn btn-sm btn-default showoverlay" id="savemodaleditdotation">Enregistrer</button>
    </div>
...
$(document).ready(function () {
    ...
    $(document).on('click','#closemodaleditdotation',function(e){
        $('#modaleditdotation').modal('toggle');
    }); 

    $('#modaleditdotation #DotationMaterielinformatiquesId').on('change',function(){
        if (this.value != ''){
            $('#modaleditdotation #DotationTypematerielId').attr('disabled', 'disabled');
        } else {
            $('#modaleditdotation #DotationTypematerielId').removeAttr('disabled');
        }
    });  
   
    $('#modaleditdotation #DotationTypematerielId').on('change',function(){
        if (this.value != ''){
            $('#modaleditdotation #DotationMaterielinformatiquesId').attr('disabled', 'disabled');
        } else {
            $('#modaleditdotation #DotationMaterielinformatiquesId').removeAttr('disabled');
        }
    }); 
});
</script>
...
if (count($this->params->data) > 0) :
    $keyword = $this->params->data['Architecture']['SEARCH'];
...
?>           
        ...
        <?php if (userAuth('profil_id')!='2' && isAuthorized('architectures', 'add')) : ?>
        ...
        <ul class="nav navbar-nav toolbar pull-right">
            <li class="dropdown">
                ...
                <ul class="dropdown-menu" style="left: -205px;min-width: 250px;max-width: 250px;">
                    <li>
                        <?php echo $this->Form->create("Architecture",array('url' => array('action' => 'search',$pass0,$pass1), 'class'=>'toolbar-form pull-right','inputDefaults' => array('error'=>false,'label'=>false,'div' => false))); ?>
...
<?php if ($this->params['action']=='index') { ?><div class="panel-body panel-filter marginbottom15 ">
    <strong>Filtre appliqué : </strong><em>Liste des architectures <?php echo $strfilter; ?></em></div><?php } ?>     
<div class="">
...
<tr>
        <td><?php echo h($architecture['Architecture']['NOM']); ?>&nbsp;</td>
        <td><?php echo h($architecture['Entite']['NOM']); ?>&nbsp;</td>
        <td><?php echo $architecture['Architecture']['COMMENTAIRE']; ?>&nbsp;</td>
        <td style="text-align:center;"><?php $image = (isset($architecture['Architecture']['ACTIF']) && $architecture['Architecture']['ACTIF']==true) ? 'ok_2' : 'ok_2 disabled' ; ?>
            <a href="#" class="actif cursor showoverlay" data-id="<?php echo $architecture['Architecture']['id']; ?>" ><span class="glyphicons <?php echo $image; ?> notchange"></span></a></td>                  
        <td class="actions">
            <?php if (userAuth('profil_id')!='2' && isAuthorized('architectures', 'view')) : ?>
            <?php echo '<span class="glyphicons eye_open" data-rel="popover" data-title="<h3>Architecture :</h3>" data-content="<contenttitle>Crée le: </contenttitle>'.h($architecture['Architecture']['created']).'<br/><contenttitle>Modifié le: </contenttitle>'.h($architecture['Architecture']['modified']).'" data-trigger="click" style="cursor: pointer;"></span>'; ?>&nbsp;
            <?php endif; ?>
            <?php if (userAuth('profil_id')!='2' && isAuthorized('architectures', 'edit')) : ?>
            <?php echo $this->Html->link('<span class="glyphicons pencil showoverlay notchange"></span>', array('action' => 'edit', $architecture['Architecture']['id']),array('escape' => false)); ?>&nbsp;
            <?php endif; ?>
            <?php if (userAuth('profil_id')!='2' && isAuthorized('architectures', 'delete')) : ?>
            <?php echo $this->Form->postLink('<span class="glyphicons bin notchange"></span>', array('action' => 'delete', $architecture['Architecture']['id']),array('escape' => false), __('Etes-vous certain de vouloir supprimer cette architecture ?')); ?>                    
            <?php endif; ?>                    
        </td>
</tr>
<?php endforeach; ?>
...
$(document).ready(function () {
    $(document).on('click','.actif',function(e){
        var id = $(this).attr('data-id');
            $.ajax({
                dataType: "html",
                type: "POST",
                url: "<?php echo $this->Html->url(array('controller'=>'architectures','action'=>'ajax_actif')); ?>/",
                data: ({id:id})
            }).done(function ( data ) {
                location.reload();
            });
    ...
    $(document).on('keyup','#ArchitectureSEARCH',function (event){
        var url = "<?php echo $this->webroot;?>architectures/search/<?php echo $pass0; ?>/";

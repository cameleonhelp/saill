...
if (count($this->params->data) > 0) :
    $keyword = $this->params->data['Composant']['SEARCH'];
...
?>     
        ...
        <?php if (userAuth('profil_id')!='2' && isAuthorized('composants', 'add')) : ?>
        ...
        <ul class="nav navbar-nav toolbar pull-right">
            <li class="dropdown">
                ...
                <ul class="dropdown-menu" style="left: -205px;min-width: 250px;max-width: 250px;">
                    <li>
                        <?php echo $this->Form->create("Composant",array('url' => array('action' => 'search',$pass0,$pass1), 'class'=>'toolbar-form pull-right','inputDefaults' => array('error'=>false,'label'=>false,'div' => false))); ?>
...
<?php if ($this->params['action']=='index') { ?><div class="panel-body panel-filter marginbottom15 ">
    <strong>Filtre appliqué : </strong><em>Liste des composants <?php echo $strfilter; ?></em></div><?php } ?>     
<div class="">
...
<tr>
    <td><?php echo h($composant['Composant']['NOM']); ?>&nbsp;</td>
    <td><?php echo h($composant['Entite']['NOM']); ?>&nbsp;</td>
    <td style="text-align:center;"><?php $image = (isset($composant['Composant']['ACTIF']) && $composant['Composant']['ACTIF']==true) ? 'ok_2' : 'ok_2 disabled' ; ?>
        <a href="#" class="actif cursor showoverlay" data-id="<?php echo $composant['Composant']['id']; ?>" ><span class="glyphicons <?php echo $image; ?> notchange"></span></a></td>             
    <td class="actions">
        <?php if (userAuth('profil_id')!='2' && isAuthorized('composants', 'view')) : ?>
        <?php echo '<span class="glyphicons eye_open" data-rel="popover" data-title="<h3>Composant :</h3>" data-content="<contenttitle>Crée le: </contenttitle>'.h($composant['Composant']['created']).'<br/><contenttitle>Modifié le: </contenttitle>'.h($composant['Composant']['modified']).'" data-trigger="click" style="cursor: pointer;"></span>'; ?>&nbsp;
        <?php endif; ?>
        <?php if (userAuth('profil_id')!='2' && isAuthorized('composants', 'edit')) : ?>
        <?php echo $this->Html->link('<span class="glyphicons pencil showoverlay notchange"></span>', array('action' => 'edit', $composant['Composant']['id']),array('escape' => false)); ?>&nbsp;
        <?php endif; ?>
        <?php if (userAuth('profil_id')!='2' && isAuthorized('composants', 'delete')) : ?>
        <?php echo $this->Form->postLink('<span class="glyphicons bin notchange"></span>', array('action' => 'delete', $composant['Composant']['id']),array('escape' => false), __('Etes-vous certain de vouloir supprimer ce composant ?')); ?>                    
        <?php endif; ?>                    
    </td>
</tr>
<?php endforeach; ?>
...
$(document).ready(function () {
    $(document).on('click','.actif',function(e){
        var id = $(this).attr('data-id');
            $.ajax({
                dataType: "html",
                type: "POST",
                url: "<?php echo $this->Html->url(array('controller'=>'composants','action'=>'ajax_actif')); ?>/",
                data: ({id:id})
            }).done(function ( data ) {
                location.reload();
            });
    ...
    $(document).on('keyup','#ComposantSEARCH',function (event){
        var url = "<?php echo $this->webroot;?>composants/search/<?php echo $pass0; ?>/<?php echo $pass1; ?>/";

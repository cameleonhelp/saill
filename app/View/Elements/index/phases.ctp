...
if (count($this->params->data) > 0) :
    $keyword = $this->params->data['Phase']['SEARCH'];
...
?>      
        ...
        <?php if (userAuth('profil_id')!='2' && isAuthorized('phases', 'add')) : ?>
        ...
        <ul class="nav navbar-nav toolbar pull-right">
            <li class="dropdown">
                ...
                <ul class="dropdown-menu" style="left: -205px;min-width: 250px;max-width: 250px;">
                    <li>
                        <?php echo $this->Form->create("Phase",array('url' => array('action' => 'search',$pass0,$pass1), 'class'=>'toolbar-form pull-right','inputDefaults' => array('error'=>false,'label'=>false,'div' => false))); ?>
...
<tr>
    <td><?php echo h($phase['Phase']['NOM']); ?>&nbsp;</td>
    <td><?php echo h($phase['Entite']['NOM']); ?>&nbsp;</td>
    <td style="text-align:center;"><?php $image = (isset($phase['Phase']['ACTIF']) && $phase['Phase']['ACTIF']==true) ? 'ok_2' : 'ok_2 disabled' ; ?>
        <a href="#" class="actif cursor showoverlay" data-id="<?php echo $phase['Phase']['id']; ?>" ><span class="glyphicons <?php echo $image; ?> notchange"></span></a></td>               
    <td class="actions">
        <?php if (userAuth('profil_id')!='2' && isAuthorized('phases', 'view')) : ?>
        <?php echo '<span class="glyphicons eye_open" data-rel="popover" data-title="<h3>Phase :</h3>" data-content="<contenttitle>Crée le: </contenttitle>'.h($phase['Phase']['created']).'<br/><contenttitle>Modifié le: </contenttitle>'.h($phase['Phase']['modified']).'" data-trigger="click" style="cursor: pointer;"></span>'; ?>&nbsp;
        <?php endif; ?>
        <?php if (userAuth('profil_id')!='2' && isAuthorized('phases', 'edit')) : ?>
        <?php echo $this->Html->link('<span class="glyphicons pencil showoverlay notchange"></span>', array('action' => 'edit',$phase['Phase']['id']),array('escape' => false)); ?>&nbsp;
        <?php endif; ?>
        <?php if (userAuth('profil_id')!='2' && isAuthorized('phases', 'delete')) : ?>
        <?php echo $this->Form->postLink('<span class="glyphicons bin notchange"></span>', array('action' => 'delete',$phase['Phase']['id']),array('escape' => false), __('Etes-vous certain de vouloir supprimer cette phase ?')); ?>                    
        <?php endif; ?>                    
    </td>
</tr>
<?php endforeach; ?>
...
$(document).ready(function () {
    $(document).on('click','.actif',function(e){
        var id = $(this).attr('data-id');
            $.ajax({
                dataType: "html",
                type: "POST",
                url: "<?php echo $this->Html->url(array('controller'=>'phases','action'=>'ajax_actif')); ?>/",
                data: ({id:id})
            }).done(function ( data ) {
                location.reload();
            });
    ...
    $(document).on('keyup','#PhaseSEARCH',function (event){
        var url = "<?php echo $this->webroot;?>phases/search/<?php echo $pass0; ?>/<?php echo $pass1; ?>/";

...
?>      
        ...
        <?php if (userAuth('profil_id')!='2' && isAuthorized('etats', 'add')) : ?>
        ...
        <ul class="nav navbar-nav toolbar pull-right">
            <li class="dropdown">
                ...
                <ul class="dropdown-menu" style="left: -205px;min-width: 250px;max-width: 250px;">
                    <li>
                        <?php echo $this->Form->create("Etat",array('url' => array('action' => 'search',$pass0,$pass1), 'class'=>'toolbar-form pull-right','inputDefaults' => array('error'=>false,'label'=>false,'div' => false))); ?>
...
<?php if ($this->params['action']=='index') { ?><div class="panel-body panel-filter marginbottom15 ">
    <strong>Filtre appliqué : </strong><em>Liste des états <?php echo $strfilter; ?></em></div><?php } ?>     
<div class="">
...
<tr>
    ...
    <th width="70px"><?php echo $this->Paginator->sort('ORDER','Ordre'); ?></th>
...
<tr>
    <td><?php echo h($etat['Etat']['NOM']); ?>&nbsp;</td>
    <td><?php echo h($etat['Entite']['NOM']); ?>&nbsp;</td>
    <td style="text-align:right;"><?php echo h($etat['Etat']['ORDER']); ?>&nbsp;</td>
    <td style="text-align:center;"><?php $image = (isset($etat['Etat']['ACTIF']) && $etat['Etat']['ACTIF']==true) ? 'ok_2' : 'ok_2 disabled' ; ?>
        <a href="#" class="actif cursor showoverlay" data-id="<?php echo $etat['Etat']['id']; ?>" ><span class="glyphicons <?php echo $image; ?> notchange"></span></a></td>             
    <td class="actions">
        <?php if (userAuth('profil_id')!='2' && isAuthorized('etats', 'view')) : ?>
        <?php echo '<span class="glyphicons eye_open" data-rel="popover" data-title="<h3>Etat :</h3>" data-content="<contenttitle>Crée le: </contenttitle>'.h($etat['Etat']['created']).'<br/><contenttitle>Modifié le: </contenttitle>'.h($etat['Etat']['modified']).'" data-trigger="click" style="cursor: pointer;"></span>'; ?>&nbsp;
        <?php endif; ?>
        <?php if (userAuth('profil_id')!='2' && isAuthorized('etats', 'edit')) : ?>
        <?php echo $this->Html->link('<span class="glyphicons pencil showoverlay notchange"></span>', array('action' => 'edit', $etat['Etat']['id']),array('escape' => false)); ?>&nbsp;
        <?php endif; ?>
        <?php if (userAuth('profil_id')!='2' && isAuthorized('etats', 'delete')) : ?>
        <?php echo $this->Form->postLink('<span class="glyphicons bin notchange"></span>', array('action' => 'delete', $etat['Etat']['id']),array('escape' => false), __('Etes-vous certain de vouloir supprimer cet état ?')); ?>                    
        <?php endif; ?>                    
    </td>
</tr>
<?php endforeach; ?>
...
$(document).ready(function () {
    $(document).on('click','.actif',function(e){
        var id = $(this).attr('data-id');
            $.ajax({
                dataType: "html",
                type: "POST",
                url: "<?php echo $this->Html->url(array('controller'=>'etats','action'=>'ajax_actif')); ?>/",
                data: ({id:id})
            }).done(function ( data ) {
                location.reload();
            });
    ...
    $(document).on('keyup','#EtatSEARCH',function (event){
        var url = "<?php echo $this->webroot;?>etats/search/<?php echo $pass0; ?>/<?php echo $pass1; ?>/";

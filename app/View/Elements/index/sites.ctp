        ...
        <ul class="nav navbar-nav toolbar pull-right">
            <li class="dropdown">
                ...
                <ul class="dropdown-menu" style="left: -205px;min-width: 250px;max-width: 250px;">
                    <li>
                        <?php echo $this->Form->create("Site",array('action' => 'search', 'class'=>'toolbar-form pull-right','inputDefaults' => array('error'=>false,'label'=>false,'div' => false))); ?>
...
<tr>
        ...
        <td class="actions">
            ...
            <?php echo '<span class="glyphicons eye_open" data-rel="popover" data-title="<h3>Site :</h3>" data-content="<contenttitle>Crée le: </contenttitle>'.h($site['Site']['created']).'<br/><contenttitle>Modifié le: </contenttitle>'.h($site['Site']['modified']).'" data-trigger="click" style="cursor: pointer;"></span>'; ?>&nbsp;
            <?php endif; ?>
            <?php if (userAuth('profil_id')!='2' && isAuthorized('sites', 'edit')) : ?>
            <?php echo $this->Html->link('<span class="glyphicons pencil showoverlay notchange"></span>', array('action' => 'edit', $site['Site']['id']),array('escape' => false)); ?>&nbsp;
            <?php endif; ?>
            <?php if (userAuth('profil_id')!='2' && isAuthorized('sites', 'delete')) : ?>
            <?php echo $this->Form->postLink('<span class="glyphicons bin notchange"></span>', array('action' => 'delete', $site['Site']['id']),array('escape' => false), __('Etes-vous certain de vouloir supprimer ce site ?')); ?>
            <?php endif; ?>
        </td>
</tr>
<?php endforeach; ?>
...
<script>   
    $(document).on('keyup','#SiteSEARCH',function (event){
        var url = "<?php echo $this->webroot;?>sites/search/";

<div class="sites index">
	...
	<tr>
		...
		<td class="actions">
                    ...
                    <?php echo '<span class="glyphicons eye_open" rel="popover" data-title="<h3>Site :</h3>" data-content="<contenttitle>Crée le: </contenttitle>'.h($site['Site']['created']).'<br/><contenttitle>Modifié le: </contenttitle>'.h($site['Site']['modified']).'" data-trigger="click" style="cursor: pointer;"></span>'; ?>&nbsp;
                    <?php endif; ?>
                    <?php if (userAuth('profil_id')!='2' && isAuthorized('sites', 'edit')) : ?>
                    <?php echo $this->Html->link('<span class="glyphicons pencil"></span>', array('action' => 'edit', $site['Site']['id']),array('escape' => false)); ?>&nbsp;
                    <?php endif; ?>
                    <?php if (userAuth('profil_id')!='2' && isAuthorized('sites', 'delete')) : ?>
                    <?php echo $this->Form->postLink('<span class="glyphicons bin"></span>', array('action' => 'delete', $site['Site']['id']),array('escape' => false), __('Etes-vous certain de vouloir supprimer ce site ?')); ?>
                    <?php endif; ?>
		</td>
	</tr>
        <?php endforeach; ?>

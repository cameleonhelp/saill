<div class="actions index" id="ActionsRefresh">
	...
	<tr>
		...
		<td style="text-align:center;"><?php echo h($action['Action']['DEBUT']); ?>&nbsp;</td>
                <?php $classtd = enretard($action['Action']['STATUT'],$action['Action']['ECHEANCE']) ? "class='td-error'" : ""; ?>
		...
		<td class="actions">
                    ...
                    <?php echo $this->Form->postLink('<i class="icon-retweet"></i>', array('action' => 'dupliquer', $action['Action']['id']),array('escape' => false), __('Etes-vous certain de vouloir dupliquer cette action ?\r\nCette action vous sera attribuée.')); ?>
                    <?php endif; ?>                        
                </td>
	</tr>
<?php endforeach; ?>

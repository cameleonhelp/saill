<div class="listediffusions index">
	...
	<tr>
		...
		<td class="actions">
                    ...
                    <?php echo '<span class="glyphicons eye_open" rel="popover" data-title="<h3>liste de diffusion :</h3>" data-content="<contenttitle>Crée le: </contenttitle>'.h($listediffusion['Listediffusion']['created']).'<br/><contenttitle>Modifié le: </contenttitle>'.h($listediffusion['Listediffusion']['modified']).'" data-trigger="click" style="cursor: pointer;"></span>'; ?>&nbsp;
                    <?php endif; ?>
                    <?php if (userAuth('profil_id')!='2' && isAuthorized('listediffusions', 'edit')) : ?>
                    <?php echo $this->Html->link('<span class="glyphicons pencil"></span>', array('action' => 'edit', $listediffusion['Listediffusion']['id']),array('escape' => false)); ?>&nbsp;
                    <?php endif; ?>
                    <?php if (userAuth('profil_id')!='2' && isAuthorized('listediffusions', 'delete')) : ?>
                    <?php echo $this->Form->postLink('<span class="glyphicons bin"></span>', array('action' => 'delete', $listediffusion['Listediffusion']['id']),array('escape' => false), __('Etes-vous certain de vouloir supprimer cette liste de diffusion ?')); ?>
                    <?php endif; ?>
		</td>
	</tr>
        <?php endforeach; ?>

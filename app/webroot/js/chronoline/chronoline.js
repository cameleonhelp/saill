...
Date.MONTH_NAMES = ['Janvier', 'Février', 'Mars', 'Avril', 'Mai', 'Juin', 'Juillet', 'Août', 'Septembre', 'Octobre', 'Novembre', 'Décembre'];
...
Date.prototype.formatDate = function(formatString){
    ...
    if(formatString.indexOf('%b') != -1){
        var month = Date.MONTH_NAMES[this.getMonth()];
...
function Chronoline(domElement, events, options) {
    ...
    //t.paper = document.createElement('span');
    ...
    t.paperElem.style.left = - (t.defaultStartDate - t.startDate) * t.pxRatio + 20 + 'px';
        //jQuery('svg').attr("tooltip-append-to-body", true);
    t.goToToday(t.today,0);

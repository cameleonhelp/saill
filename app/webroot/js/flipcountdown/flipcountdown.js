					...
					for( var i = 0, n = chars.length;i<n;i++ ){
						...
						if( blocks[i].data('value')!=chars[i] ){
							if( !_typeCompare(blocks[i].data('value'),chars[i]) ){
								blocks[i]
                                                                        //DO: [JLR] personnalisation pour afficher la date en ajoutant la classe xdsoft_break css modifié en conséquence
									.removeClass('xdsoft_separator')
									.removeClass('xdsoft_dot')
                                                                        .removeClass('xdsoft_break');
								switch( chars[i] ){
                                                                        case '/':blocks[i].addClass('xdsoft_break');break;
									...
									case ' ':blocks[i].addClass('xdsoft_space');break; 
                                                                ...
                                                                $('.xdsoft_break').html('/').removeClass('xdsoft_digit');
							}
							if( !isNaN(chars[i]) ){
				...
				_calcMoment = function(){
					...
					if( typeof value!=='undefined' ){
						switch( value.constructor ){
							...
							break;
							case String:
                                                                //DO: [JLR] personnalisation du regex pour afficher la date

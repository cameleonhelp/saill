...
tinymce.init({ 
   ...
   statusbar:false,
   skin : 'flat',
   plugins: "hr link textcolor table code searchreplace responsivefilemanager insertdatetime",
   menubar: "file edit insert format table tools",
   toolbar: "undo redo | bold italic | hr link unlink | forecolor | backcolor | numlist bullist | alignleft aligncenter alignright alignjustify | responsivefilemanager  | searchreplace | insertdatetime",
   insertdatetime_formats: ["%d/%m/%Y","%d/%m/%Y : ","%d/%m/%Y à %H:%M:%S","%d/%m/%Y à %H:%M:%S : "],
   height : "300",
   relative_urls: false,
   convert_urls: false,

...
class FacturationsController extends AppController {
    ...
    /**
     * renvois le périmétre de visibilité
     * 
     * @return string
     */
    ...
    /**
     * filtre la facturation par utilisateur
     * 
    ...
    /**
     * filtre la facturation sur le mois et l'année
     * 
     * @param string $mois
     * @param string $annee
     * @return array
    ...
    /**
     * filtre la facturation visible et non visible
     * 
    ...
    /**
     * filtre la facturation sur les activité d'indisponibilité
     * 
     * @param boolean $indisponibilite
     * @return array
     */
    ...
    /**
     * renvois les facturations groupées par date utilisateur et version
     * 
    ...
    /**
     * renvois la liste des années des facturations pour renseigner le menu
     * 
     * @return array
     */
    ...
    /**
     * mets en cache les valeurs de l'export
     * 
     * @param array $condition
     */
    ...
    /**
     * chargement des saisie faites par les agents pour les transformer en facturation
     * @param string $userid
     * @param string $reelid
     */
    public function add($userid=null,$reelid=null) {
     ...
     /**
      * Modification d'une facturation existante
      * 
      * @param string $id
      * @param string $userid
      */
    ...
    /**
     * suppression de ligne de facturation
     * 
     * @param string $id
    ...
    public function delete($id = null) {
        ...
        if (!$this->Facturation->exists()) {
                throw new NotFoundException(__('Facturation incorrecte'));
        }
        if ($this->Facturation->delete()) {
                $this->Session->setFlash(__('Facturation supprimée',true),'flash_success');
                $this->History->goBack(1);
        }
        $this->Session->setFlash(__('Facturation <b>NON</b> supprimée',true),'flash_failure');
        $this->History->goBack(1);
    }

    /**
     * recherhce de facturation
     * 
     * @param string $utilisateur
     * @param string $mois
     * @param string $visible
        ...
        if (isAuthorized('facturations', 'index')) :
            ...
            if($keywords!= ''):
                ...
                foreach ($arkeywords as $key=>$value):
                    $ornewconditions[] = array('OR'=>array("Facturation.VERSION = '".$value."'","Activite.NOM LIKE '%".$value."%'","Utilisateur.NOM LIKE '%".$value."%'","Utilisateur.PRENOM LIKE '%".$value."%'"));
                endforeach;
    ...
    /**
     * renvois la saisie de l'agent à une date donnée
     * 
     * @param string $userid
     * @param string $date
     * @return array
    ...
    /**
     * enregistre la facturation sur ajout ou modification
     */
                        ...
                        if (is_array($facturation)):
                            ...
                            if ($this->Facturation->save($facturation)) {
                                ...
                                if (isset($facturation['VERSION']) && intval($facturation['VERSION']) > 0):
                                    ...
                                    if(isset($facturation['OldId'])):
                                        $this->Facturation->id = $facturation['OldId'];                                         
    ...
    /**
     * renvois les anciennes facturation
     * 
    ...
    public function getOldFacturationId($utilisateur_id,$date,$activite_id,$version){
        ...
        $facturation = isset($oldId['Facturation']['id']) ? $oldId['Facturation']['id'] : null;
        return $facturation;
    }
        
    /**
     * renvois la liste des responsables de saisie
     * 
     * @param string $visibility
     * @return array
     */
    ...
    /**
     * renvois la saisie en trop de la première semaine
     * 
     * @param array $activitefirstweek
     * @return array
     */
    ...
    /**
     * renvois la saisie en trop de la dernière semaine
     * 
     * @param array $activitelastweek
     * @return array
     */
    ...
    /**
     * fusion et somme d'un array par projet/activite/utilisateur
     * 
     * @param array $array
     * @return array
     */
    ...
    /**
     * fusion et somme par projet d'un array
     * 
     * @param array $array
     * @return array
     */
    ...
    /**
     * fusion et somme par projet/activité d'un array
     * 
     * @param array $array
     * @return array
     */
    ...
    /**
     * facturation pour les cheminots
     * 
     * @param array $options
     * @return array
     */
    public function forsncf($options=null){
        ...
        if($utilisateurs != '' && $utilisateurs != '0'):
            ...
            $byprojetactiviteutilisateur = $this->array_sum_merge($entrop);
            $indisponibilite = $options['indisponibilite'] ? '1=1': 'Activite.projet_id > 1';
            $result = $this->Facturation->find('all',array('fields'=>array('MONTH(Facturation.DATE) AS MONTH', 'YEAR(Facturation.DATE) AS YEAR','Activite.id','Activite.NOM','Activite.projet_id','SUM(Facturation.TOTAL) AS NB','Utilisateur.id','Utilisateur.username','Utilisateur.NOM','Utilisateur.PRENOM','Activite.projet_id','Utilisateur.societe_id','Activite.NUMEROGALLILIE'),'conditions'=>array($destinataire,$periode,$indisponibilite,'Facturation.VISIBLE'=>0),'order'=>array('Utilisateur.NOM'=>'asc','Utilisateur.PRENOM'=>'asc','Facturation.utilisateur_id'=>'asc'),'group'=>array('Facturation.utilisateur_id','Activite.projet_id','Activite.NOM'),'recursive'=>0));
            return array('trop'=>$byprojetactiviteutilisateur,'result'=>$result);        
        endif;
    }

    /**
     * renvois la saisie en trop de la première semaine pour les prestataires
     * 
     * @param array $activitefirstweek
     * @param string $mois
     * @return array
    ...
    /**
     * renvois la saisie en trop sur la dernière semaine pour les cheminots
     * 
     * @param array $activitelastweek
     * @param string $du fin de la période
     * @param string $au début de la période
     * @return array
     */
    ...
    /**
     * renvois les saisies en trop sur la première semaine pour les cheminots
     * 
     * @param array $activitefirstweek
     * @param string $du date de fin de la plage
     * @return array
     */
    ...
    /**
     * renvois les jours en trop de la dernière semaine pour les prestataires
     * 
     * @param array $activitelastweek
     * @param string $mois
     * @return array
    ...
    /**
     * fusion et addition de dans un array
     * 
     * @param array $array
     * @return array
     */

...
class UtilisateursController extends AppController {
    ...
    /**
     * méthode permettant ici d'autoriser l'utilisation de certaines méthodes sans authentification et le hashage du cookies
     */
    ...
    /**
     * Méthode qui permet de trouver la limite de visibilité de l'utilisateur
     * 
     * @return string
     */
    ...
    /**
     * filtre les utilisateur par état
     * 
     * @param int $id
    ...
    public function get_utilisateur_etat_filter($id,$visibility) {
        ...
        switch ($id){
            ...
            case 'inactif':
                ...
                elseif ($visibility!=''):
                    $result['condition']="Utilisateur.ACTIF=0 AND (Utilisateur.profil_id > 0 OR Utilisateur.profil_id=-2)"; // retiré la condition sur Utilisateur.id IN (".$visibility.") AND 
                else:
    ...
    /**
     * filtre les utilisateurs par section
     * 
     * @param int $id
     * @param string $etat
     * @return string
    ...
    /**
     * filtre les utilisateur par société
     * 
     * @param int $id
     * @return string
     */
    ...
    /**
     * filtre les utilisateur par la première lettre du nom
     * 
     * @param string $id
     * @return string
     */
    ...
    /**
     * mets en variable de session l'export de l'affichage
     * 
    ...
    /**
     * donne la liste des utilisateur hiérarchique
     * 
     * @return array
     */
    ...
    /**
     * liste des hiérarchique en fonction de mon cercle de visibilité
     * 
     * @param string $visibility
     * @return array
     */
    ...
    /**
     * retourne une string des sections des utilisateur
     * 
     * @return string
     */
     ...
     * 
     * @param string $filtreUtilisateur
     * @param string $filtreSection
     * @param string $filtresociete
     * @param string $filtreAlpha
    ...
    /**
     * sauvegarde de l'historique de l'action réalisée sur le compte de l'utilisateur
     * 
     * @param int $id
     * @param string $msg
     */
    ...
    /**
     * supprime l'association sur l'entité pour cet utilisateur
     * 
     * @param int $id
     */
    ...
    /**
     * return le profil de l'utilisateur et tous les objets s'y référant
     *
     * @throws NotFoundException
     * @throws MethodNotAllowedException
     * @param int $id
...
//                    ldap_unbind($conn);
                    ...
                    CakeLog::write('warning-login', 'Tentative de connexion échouée avec le login : '.$username.' login inconnu du LDAP','login_error');
            ...
            if($this->Auth->request->data['Utilisateur']['username']=="0000000A"):
                ...
                if (count($this->Utilisateur->find('first', array('conditions'=>array('Utilisateur.ACTIF'=>1,'Utilisateur.username'=>$username))))>0){
                    if (count($this->Utilisateur->find('first', array('conditions'=>array('Utilisateur.ACTIF'=>1,'Utilisateur.username'=>$username,'Utilisateur.password'=>$password))))>0) {
                        ...
                        CakeLog::write('login', 'Connexion avec succès de '.$username,'login_success');
                    ...
                    CakeLog::write('warning-login', 'Tentative de connexion échouée avec le login : '.$username.' login inexistant ou invalide','login_error');
            ...
            if (!is_null($cookie)) {
                ...
                if($utilisateur['password']=='ldap'):
                    if (count($this->Utilisateur->find('first', array('conditions'=>array('Utilisateur.ACTIF'=>1,'Utilisateur.username'=>$utilisateur['username']))))>0){
                        if($utilisateur['username'] != "0000000A"):
                            $utilisateur = $this->Utilisateur->find('first', array('conditions'=>array('Utilisateur.ACTIF'=>1,'Utilisateur.username'=>$utilisateur['username'])));
                        else:
                        ...
                        $profil = isset($utilisateur['Utilisateur']['profil_id']) ? $utilisateur['Utilisateur']['profil_id'] : 2 ;
                        $autorisations = $this->Utilisateur->Profil->Autorisation->find('all',array('conditions'=>array('Autorisation.profil_id'=>$profil)));
                        $this->Session->renew();
                        $this->Session->write(AUTHORIZED,$autorisations);
                        if ($this->Auth->login($utilisateur['Utilisateur'])) {
                            CakeLog::write('login', 'Connexion avec cookie avec succès de '.$utilisateur['Utilisateur']['username'],'login_success');
                    ...
                    $this->Session->renew();
                    $this->Cookie->delete('remember_me_cookie');
                    CakeLog::write('warning-login', 'Tentative de connexion échouée avec le login : '.$username.' login inexistant ou invalide avec cookies','login_error');
                    $this->Session->setFlash(__('Cookie : Login inexistant ou compte invalide, contacter l\'administrateur',true),'flash_failure');
                    } 
                endif;
            }
    ...
    /**
     * duplique le compte de l'utilisateur en un nouveau compte
     *
     * @throws NotFoundException
     * @throws MethodNotAllowedException
     * @param int $id
    ...
    /**
     * Initialisation du mot de passe [OBSOLETE suite à la mise en place de la connexion LDAP]
     *
     * @throws NotFoundException
     * @throws MethodNotAllowedException
     * @param int $id
    ...
    /**
     * Initialisation de mon mot de passe [OBSOLETE suite à la mise en place de la connexion LDAP]
     *
     * @throws NotFoundException
     * @throws MethodNotAllowedException
     * @param int $id
    ...
    /**
     * calcul les Conges d'un utilisateur
     * 
     * @param int $id
     * @return array
     */        
    ...
    /**
     * calcul les VT d'un utilisateurVT
     * 
     * @param type $id
     * @return array
     */        
    ...
    /**
     * 
     * @return typeprolonge les comptes des utilisateurs sélectionnés et envois un émail
     */
    ...
    /**
     * désactivation du compte d'un utilisateur ou de plusieurs
     */
    ...
    /**
     * ajout d'un nouvelle action suite à la création d'un nouvel utilisateur
     * 
     * @param int $id
     */
    ...
    /**
     * envois d'un mail suite à la création d'un nouvel utilisateur
     * 
    ...
    /**
     * envois d'un mail au gestionnaire d'annuaire
     * 
     * @param int $id
     */
    ...
    /**
     * envois un mail à l'utilisateur suite à la génération automatique d'un mot de passe
     * 
    ...
    /**
     * envois un mail pour la prolongation d'un compte
     * 
     * @param int $utilisateur
     */
    ...
    /**
     * retourne les informations utilisateur au format json
     * 
     * @param int $id
     * @return json
     */
    ...
    /**
     * retourne la liste des utilisateurs d'une liste de société
     * 
     * @return array
     */
    ...
    /**
     * Retourne dans quel état doit être le menu
     * 
     * @return boolean
     */
    ...
    /**
     * garde en base si le menu doit ou non être affcihé [OBSOLETE suite au changement d'interface]
     */
    ...
    /**
     * fait progresser l'engagaement de confidentialité en fonction de l'ancien état
     * 
     * @param int $id
     * @param boolean $loop
     * @throws NotFoundException
    ...
    /**
     * change l'engagaement de confidentialité des utilisateurs sélectionnés
     * 
     * @return void
     */
    ...
    /**
     * export de la fiche mouvement au format Excel
     * 
     * @param type $id
     */
    ...
    /**
     * retourne la liste des utilisateurs admin et/ou générique
     * 
     * @param boolean $admin
     * @param boolean $generique
     * @return array
     */
    ...
    /**
     * retourne la liste des utilisateur admin et/ou générique
     * 
     * @param boolean $admin
     * @param boolean $generique
     * @return array
    ...
    /**
     * retourne la liste des utilisateurs générique ou pas
     * 
     * @param int $generique
     * @return array
     */
    ...
    /**
     * retourne les émail des utilisateurs
     * 
     * @param string $list
     * @return string
     */
    ...
    /**
     * retourne les nom long de tous les utilisateurs
     * 
     * @param string $list
     * @return string
     */
    ...
    /**
     * trouve le nom long d'un utilisateur
     * 
     * @param int $id
     * @return string
     */
    ...
    /**
     * sauvegarde du mot de passe [OBSOLETE depuis la mise en place de LDAP]
     * 
     * @return string
     */
    ...
    /**
     * liste des utilisateurs appartenant aux mêmes cercles d'un utilisateur en fonction de sa visibilité
     * 
    ...
    /**
     * désactivation des comptes ayant expirés
     * 
     * @return void
     */
    ...
    /**
     * liste les utilisateur utilisant le gestionnaire d'aébsences
     * 
     * @param int $actif
     * @return array
     */
    ...
    /**
     * liste des utilisateur utilisant le gestionnaire d'absences
     * 
     * @param int $actif
     * @return string
     */
    ...
    /**
     * création d'un utilisateur forcé lors de l'import des CRA
     * 
     * @return int
     */
    public function import_add_forced(){
        $user_id = $this->exist($username);
        if($user_id != false):
            $tmp = array();
            $objSociete = new SocietesController();
            $tmp['Utilisateur']['societe_id']=$objSociete->idgetbynom(); //récupéré à partir de l'import
            $tmp['Utilisateur']['password']='';
            $tmp['Utilisateur']['ACTIF']=1;
            $tmp['Utilisateur']['NAISSANCE']=date('Y-m-d');
            $tmp['Utilisateur']['NOM']=''; //récupéré à partir de l'import
            $tmp['Utilisateur']['PRENOM']=''; //récupéré à partir de l'import
            $tmp['Utilisateur']['HIERARCHIQUE']=0;
            $tmp['Utilisateur']['GESTIONABSENCES']=0;
            $tmp['Utilisateur']['WIDEAREA']=0;
            $tmp['Utilisateur']['NOTIFYME']=0;
            $tmp['Utilisateur']['profil_id']=2; //forcé à visiteur
            $tmp['Utilisateur']['section_id']=''; //forcé à vide
            $tmp['Utilisateur']['username']=''; //récupéré à partir de l'import
            $tmp['Utilisateur']['created']=date('Y-m-d');
            $tmp['Utilisateur']['modified']=date('Y-m-d');
            $this->Utilisateur->create();
            $this->Utilisateur->save($tmp);
            return $this->Utilisateur->getLastInsertID();
        else:
            return $user_id;
        endif;
    }
